...
export function CertificationsListContent() {
  ...
  const { mutate: startCertMutate } = useStartCertification();

  const startCert = (slug: string) => {
    setStartingSlug(slug);
    startCertMutate(slug, { onSettled: () => setStartingSlug(null) });
  };
  const { t } = useT();
  ...
  return (
    <div className="flex-1 p-6 overflow-auto">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-foreground">{t("certifications.title")}</h1>
        ...
        <Card className="bg-card border-border">
          <CardContent className="p-4 flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground">{t("certifications.completed")}</p>
            </div>
          </CardContent>
        </Card>
        <Card className="bg-card border-border">
          <CardContent className="p-4 flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground">{t("certifications.inProgress")}</p>
            </div>
          </CardContent>
        </Card>
        <Card className="bg-card border-border">
          <CardContent className="p-4 flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground">{t("certifications.avgScore")}</p>
            </div>
          </CardContent>
        </Card>
      </div>
      ...
      {error && (
        <div className="flex items-center gap-3 p-4 bg-destructive/10 border border-destructive/20 rounded-lg text-destructive">
          ...
          <span>{t("certifications.errorLoad")}</span>
        </div>
      )}
                  ...
                  {isInProgress && (
                    <div className="mb-4">
                      <div className="flex justify-between text-sm mb-1">
                        <span className="text-muted-foreground">{t("certifications.awaitingReview")}</span>
                      </div>
                  ...
                  {submissionsLoading ? (
                    ...
                    <Link href={`/dashboard/certifications/${cert.slug}`}>
                      <Button className="w-full cursor-pointer" variant="outline">
                        {t("certifications.viewCert")} <ArrowRight className="w-4 h-4 ml-2" />
                      </Button>
                    </Link>
                  ) : isInProgress ? (
                    <Link href={`/dashboard/certifications/${cert.slug}`}>
                      <Button className="w-full bg-primary hover:bg-primary/90 cursor-pointer">
                        {t("certifications.continue")} <ArrowRight className="w-4 h-4 ml-2" />
                      </Button>
                    </Link>
                  ) : (
                    <Button
                      className="w-full bg-primary hover:bg-primary/90 cursor-pointer"
                      disabled={startingSlug === cert.slug}
                      onClick={() => startCert(cert.slug)}>
                      {startingSlug === cert.slug ? (
                        <Loader2 className="w-4 h-4 mr-2 animate-spin" />
                      ) : (
                        <ArrowRight className="w-4 h-4 ml-2" />
                      )}
                      {t("certifications.start")}
                    </Button>
                  )}
                </CardContent>
              </Card>
